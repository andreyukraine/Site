...
$order_section_list = (@$_GET['order'] ? $_GET['order'] : 'desc');
$sort_arr = array(
    array(
        'id'   => 1,
        'name' => 'новизне',
...
if (!empty($arResult['ITEMS']))
{
    ...
    if (!empty($arResult['SKU_PROPS']))
    {
        foreach ($arResult['SKU_PROPS'] as &$arProp)
        {
            ...
            if ('TEXT' == $arProp['SHOW_MODE'])
            {
                ...
                $templateRow .= '<div class="' . $strClass . '" id="#ITEM#_prop_' . $arProp['ID'] . '_cont">' . '<span class="bx_item_section_name_gray">' . htmlspecialcharsex($arProp['NAME']) . '</span>' . '<div class="bx_size_scroller_container"><div class="bx_size"><ul id="#ITEM#_prop_' . $arProp['ID'] . '_list" style="width: ' . $strWidth . ';">';
            ...
            elseif ('PICT' == $arProp['SHOW_MODE'])
            {
                ...
                $templateRow .= '<div class="' . $strClass . '" id="#ITEM#_prop_' . $arProp['ID'] . '_cont">' . '<span class="bx_item_section_name_gray">' . htmlspecialcharsex($arProp['NAME']) . '</span>' . '<div class="bx_scu_scroller_container"><div class="bx_scu"><ul id="#ITEM#_prop_' . $arProp['ID'] . '_list" style="width: ' . $strWidth . ';">';
                foreach ($arProp['VALUES'] as $arOneValue)
                {
                    $arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
                    $templateRow .= '<li data-treevalue="' . $arProp['ID'] . '_' . $arOneValue['ID'] . '" data-onevalue="' . $arOneValue['ID'] . '" style="width: ' . $strOneWidth . '; padding-top: ' . $strOneWidth . ';"><i title="' . $arOneValue['NAME'] . '"></i>' . '<span class="cnt"><span class="cnt_item" style="background-image:url(\'' . $arOneValue['PICT']['SRC'] . '\');" title="' . $arOneValue['NAME'] . '"></span></span></li>';
                }
                $templateRow .= '</ul></div>' . '<div class="bx_slide_left" id="#ITEM#_prop_' . $arProp['ID'] . '_left" data-treevalue="' . $arProp['ID'] . '" style="' . $strSlideStyle . '"></div>' . '<div class="bx_slide_right" id="#ITEM#_prop_' . $arProp['ID'] . '_right" data-treevalue="' . $arProp['ID'] . '" style="' . $strSlideStyle . '"></div>' . '</div></div>';
            }
        ...
        <div class="sort_wrapper">
            ...
            <ul class="sort_list_section">
                <?php foreach ($sort_arr as $sa)
                {
                    $get = $_GET;
                    $get['sort'] = $sa['id'];
                    $get['order'] = (($order_section_list == 'desc') ? 'asc' : 'desc');
                    ?>
                    <li class="sort_<?php echo $sa['id'] ?>">
                        <a href="?<?php echo http_build_query($get); ?>"><?php echo $sa['name']; ?></a>
                    </li>
                <?php } ?>
            </ul>
        </div>
    ...
    <div class="section-items1">
        ...
        {
            $section = CIBlockElement::GetList(array(), array('ID' => $arItem['ID']), false, false, array('IBLOCK_SECTION_ID'))
                                     ->Fetch();
            $section = $section["IBLOCK_SECTION_ID"];
            $arSections = CIBlockSection::GetNavChain(false, $section)
                                        ->Fetch();
            ...
            <? //if($USER->IsAdmin()) {echo '<pre>'; print_r($arItem); echo '</pre>';} ?>
                <div class="col-xs-3 img_box">
                    <div>
                        ...
                        { ?>
                        <a href="<? echo $arItem['DETAIL_PICTURE']['SRC']; ?>"
                           class="fancybox">
                            <? } ?>
                            <img src="<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>" <? echo (in_array('101', $arSections)) ? 'class="ebookimg"' : ''; ?>
                                 alt=""/>
                            <? if (is_array($arItem['DETAIL_PICTURE']) && isset($arItem['DETAIL_PICTURE']['SRC']))
                            { ?>
                        </a>
                ...
                <div class="col-xs-6 item_desc">
                    ...
                    <?php if (!empty($arItem['PROPERTIES']['ARTNUMBER']['VALUE']))
                    { ?>
                        <div class="artnumber">
                           <p>Артикул: <?php echo $arItem['PROPERTIES']['ARTNUMBER']['VALUE']; ?></p>
                        </div>
                    <?php } ?>
                    ...
                    <div class="more">
                        <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"
                           title="<? echo $productTitle; ?>">Подробнее...
                        </a>
                    </div>
                </div>
                <div class="col-xs-3 price_block no-padding">
                <? //if($USER->IsAdmin()) {echo '<pre>'; print_r($arItem['PROPERTIES']['PRICE_OLD']); echo '</pre>';} ?>
                    <div class="bx_catalog_item_price">
                        ...
                        { ?>
                            <div id="<? echo $arItemIDs['PRICE']; ?>"
                                 ...
                                 <? //if($USER->IsAdmin()) {echo '<pre>'; print_r($arItem['MIN_PRICE']['VALUE']); echo '</pre>';} ?>
                                 
                                 
                                <? if (!empty($arItem['PROPERTIES']['PRICE_OLD']['VALUE']) && $arItem['PROPERTIES']['PRICE_OLD']['VALUE'] != '0.00' && site::ConvertPrice($arItem['PROPERTIES']['PRICE_OLD']['VALUE']) != site::ConvertPrice($arItem['MIN_PRICE']['VALUE']))
                                { ?>
                                ...
                                { ?>
                                    ...
                                    <? echo GetMessage('CT_BCS_TPL_MESS_PRICE_SIMPLE_MODE', array(
                                      '#PRICE#'   => str_raplace(" ", "", site::ConvertPrice($arItem['MIN_PRICE']['VALUE'])),
                                ...
                                { ?>
                                    <? echo CCurrencyLang::CurrencyFormat(str_replace(" ", "", site::ConvertPrice($arItem['MIN_PRICE']['VALUE'])), site::getCurrency(), true); ?>
                             <? } ?>
                             
                                    <? if ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $arItem['MIN_PRICE']['VALUE'])
                                { ?>
                                    <span><? echo CCurrencyLang::CurrencyFormat(str_replace(" ", "", site::ConvertPrice($arItem['MIN_PRICE']['VALUE'])), site::getCurrency(), true); ?></span>
                                <? } ?>
                                <? } ?>
                            </div>
                        <? } ?>
                    ...
                    <div class="bx_catalog_item_controls">
                        ...
                        { /* Remove comment to use product quantity ?>
                            ...
                            <div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>"
                                ...
                                { ?>
                                    <?php if ($checkcart->has_book && preg_match('~^/catalog/ebooks/~', $arItem['DETAIL_PAGE_URL']))
                                { ?>
                                    <button class="bx_bt_button bx_medium"
                                            onclick="alert('В корзине одновременно не может находится электронный и обычный товар. Пожалуйста, завершите оформление предыдущего заказа.');"></button>
                                <?php }
                                else
                                {
                                    if ($checkcart->has_ebook && !preg_match('~^/catalog/ebooks/~', $arItem['DETAIL_PAGE_URL']))
                                    { ?>
                                        <button class="bx_bt_button bx_medium"
                                                onclick="alert('В корзине одновременно не может находится электронный и обычный товар. Пожалуйста, завершите оформление предыдущего заказа.');"></button>
                                    <?php }
                ...
                <div class="bx_catalog_item_container"
                       ...
                       title="<? echo $imgTitle; ?>">
                        ...
                        { ?>
                            <div id="<? echo $arItemIDs['DSC_PERC']; ?>"
                                 ...
                                 style="display:<? echo(0 < $arItem['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;">-<? echo $arItem['MIN_PRICE']['DISCOUNT_DIFF_PERCENT']; ?>%
                            </div>
                        <? } ?>
                        <? if ($arItem['LABEL'])
                        { ?>
                    ...
                    <div class="bx_catalog_item_price">
                        <div id="<? echo $arItemIDs['PRICE']; ?>"
                            ...
                            { ?>
                                <s class="price-old"><?php echo str_replace(" ", "", site::ConvertPrice($arItem['PROPERTIES']['PRICE_OLD']['VALUE'])), ' ', $arParams['CURRENCY']; ?></s>
                            <? } ?>
                            <? if (!empty($arItem['MIN_PRICE']))
                            { ?>
                                <? if ('N' == $arParams['PRODUCT_DISPLAY_MODE'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
                            ...
                            <? } ?>
                                <? echo ' ' . $arParams['CURRENCY']; ?>
                                <? if ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $arItem['MIN_PRICE']['VALUE'])
                            { ?>
                                <span><? echo str_replace(" ", "", site::ConvertPrice($arItem['MIN_PRICE']['PRINT_VALUE']));
                                    echo ' ' . $arParams['CURRENCY']; ?></span>
                            <? } ?>
                            <? } ?>
                        </div>
                    </div>
...
<script>
    $(function()
      {
          var sort = '<? if (isset($sort_section_list))
          {
              echo strtolower($sort_section_list);
          }
          else
          {
              echo "undefined";
          } ?>';
          var order = '<? if (isset($order_section_list))
          {
              echo strtolower($order_section_list);
          }
          else
          {
              echo "undefined";
          ...
          $(document)
              .on('click', '.sort_selector_span', function()
              {
                  $(this)
