		...
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('current_price_' + id))
				    var current_price_new = document.getElementById('current_price_' + id);
					BX('current_price_' + id).innerHTML = current_price_new.innerHTML; 

				if (BX('old_price_' + id))
					BX('old_price_' + id).innerHTML = (item.FULL_PRICE_FORMATED != item.PRICE_FORMATED) ? item.FULL_PRICE_FORMATED : '';

				if (BX('sum_' + id))
					var total_new = current_price_new.innerHTML * item.QUANTITY;
					BX('sum_' + id).innerHTML = total_new.toFixed(2);
