...
// свой кастомный костыль на фильтр
$FilterString = $arParams['SECTION_ID'];
	
if (!empty($arResult['ITEMS'])) {
	
	foreach($arResult['ITEMS'] as $preItemKey => $preItem) {
		
		$strParentId = $preItem['IBLOCK_SECTION_ID'];
		
		$arrParentNode;
		
		// цикл поиска родительского раздела пока не достигнут верхний раздел
		while ($arrParentNode['DEPTH_LEVEL'] != 1) {
			
			//получаем список вложености
			$tt = CIBlockSection::GetList(array(),array('ID'=>$strParentId) );
			
			//получаем родителя
			$arrParentNode = $tt->GetNext();
			
			//устанавливаем родителя id на следущий цикл
			$strParentId = $arrParentNode['IBLOCK_SECTION_ID'];

		}
		
		// если раздел элемента совпадает и не превышено количество пишем в массив ответа
		if ( $arrParentNode['ID'] == $FilterString && count($arrNewFilterResult) < 10 ) {
			
			$arrNewFilterResult[$preItemKey] = $preItem;
			
		}
		
		unset($arrParentNode); // если цикл пройден чистим раздел
		
	} // endforeach
	
	$arResult['ITEMS'] = $arrNewFilterResult; // записываем масив ответа фильтра в результаты
	
}
// конец кастомного костыля 
