...
<? if (!empty($arResult['ITEMS'])): ?>
	...
	<div class="bx_item_list_title">
		<? if(in_array(101, $arSections)) {?>Электронная версия
		<?php } else if(in_array(433, $arSections)) { ?>Бумажная версия
		<?php } else echo GetMessage('CATALOG_RECOMMENDED_PRODUCTS_HREF_TITLE') 
	?></div>
	...
	<a id="<? echo $arItemIDs['PICT']; ?>"
		...
		class="bx_catalog_item_images <? if(in_array(101, $arSections)) { echo 'bx_catalog_item_images_ebook'; } ?>"
			...
			if (!empty($arItem['MIN_PRICE']))
			{
				if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
				{
					echo GetMessage(
						...
						array(
							'#PRICE#' => CCurrencyLang::CurrencyFormat(site::ConvertPrice($arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']), site::getCurrency(), true),
							'#MEASURE#' => GetMessage(
				...
				{
					echo CCurrencyLang::CurrencyFormat(site::ConvertPrice($arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']), site::getCurrency(),true);
				}
				if ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $arItem['MIN_PRICE']['VALUE'])
				{
					?> <span
					style="color: #a5a5a5;font-size: 12px;font-weight: normal;white-space: nowrap;text-decoration: line-through;">
					<? echo CCurrencyLang::CurrencyFormat(site::ConvertPrice($arItem['MIN_PRICE']['PRINT_VALUE']), site::GetCurrency(), true); ?></span><?
				}
			}
			?></div>
	</div><?
