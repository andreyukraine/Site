...
<div class="blog-comments" id="blg-comment-<?= $arParams["ID"] ?>">
	...
	?>
		<a name="<?= $comment["ID"] ?>"></a>
		<div class="blog-comment tab-body-container-review"
		     style="padding-left:<?= $paddingSize ?>em; <? if ($paddingSize > 0) {
			...
			<div id="blg-comment-<?= $comment["ID"] ?>">
				...
				<div class="blog-comment-cont<?= $aditStyle ?>">
					<div class="blog-comment-cont-white tab-body-container-review-content">
						<div class="review_photo">
							<img src="<?= $comment['AVATAR_file']['src'] ?>" alt="">
						</div>
						...
						*/ ?>
								...
								if (strlen($comment["urlToBlog"]) > 0) {
									?>
									<div class="blog-author review_name">
										<?
								...
								<?endif;
									...
									?>
									<div class="blog-author review_name"><?= $comment["AuthorName"] ?></div>
									<?
								}
								...
								?>
								<div class="blog-comment-date review_data">(<?= $comment["DateFormated"] ?>)</div>
							</div>
							<div class="blog-clear-float"></div>
							<div class="blog-comment-content main_review">
								...
								?>
								<div class="blog-comment-meta review_buttons_wrap">
									<?
									if ($arParams["SHOW_RATING"] == "Y") {
										?>
							...
							<? $GLOBALS["APPLICATION"]->IncludeComponent(
								...
								array("HIDE_ICONS" => "Y")
									...
									if ($bCanUserComment === true) {
										...
										<span class="blog-comment-answer"><a href="javascript:void(0)"
										                                     onclick="return showComment('<?= $comment["ID"] ?>')"><?= GetMessage("B_B_MS_REPLY") ?></a></span>
										<?
									}
									...
									if ($comment["CAN_EDIT"] == "Y") {
										...
										<span class="blog-comment-edit"><a href="javascript:void(0)"
										                                   onclick="return editComment('<?= $comment["ID"] ?>')"><?= GetMessage("BPC_MES_EDIT") ?></a></span>
										<?
									}
								...
								<? if ($arParams["AJAX_POST"] == "Y"): ?>
											<a href="javascript:void(0)"
											   onclick="return hideShowComment('<?= $comment["urlToHide"] . "&" . bitrix_sessid_get() ?>&IBLOCK_ID=<?= $_REQUEST["IBLOCK_ID"] ?>&ELEMENT_ID=<?= $_REQUEST["ELEMENT_ID"] ?>', '<?= $comment["ID"] ?>');"
											   title="<?= GetMessage("BPC_MES_HIDE") ?>">
								<? else: ?>
												<a href="<?= $comment["urlToHide"] . "&" . bitrix_sessid_get() ?>&IBLOCK_ID=<?= $_REQUEST["IBLOCK_ID"] ?>&ELEMENT_ID=<?= $_REQUEST["ELEMENT_ID"] ?>"
												   title="<?= GetMessage("BPC_MES_HIDE") ?>">
								<?endif;
								?>
								...
								<? if ($arParams["AJAX_POST"] == "Y"): ?>
											<a href="javascript:void(0)"
											   onclick="if(confirm('<?= GetMessage("BPC_MES_DELETE_POST_CONFIRM") ?>')) deleteComment('<?= $comment["urlToDelete"] . "&" . bitrix_sessid_get() ?>&IBLOCK_ID=<?= $_REQUEST["IBLOCK_ID"] ?>&ELEMENT_ID=<?= $_REQUEST["ELEMENT_ID"] ?>', '<?= $comment["ID"] ?>');"
											   title="<?= GetMessage("BPC_MES_DELETE") ?>">
								<? else: ?>
												<a href="javascript:if(confirm('<?= GetMessage("BPC_MES_DELETE_POST_CONFIRM") ?>')) window.location='<?= $comment["urlToDelete"] . "&" . bitrix_sessid_get() ?>&IBLOCK_ID=<?= $_REQUEST["IBLOCK_ID"] ?>&ELEMENT_ID=<?= $_REQUEST["ELEMENT_ID"] ?>'"
												   title="<?= GetMessage("BPC_MES_DELETE") ?>">
								<?endif;
								?>
	...
	?>
		<div class="blog-comment-nav">
			...
			?>
			<span>
							<a href="<?= $arResult["NEW_PAGES"][1] ?>" onclick="return bcNav('1', this)"
							   id="blog-comment-nav-bstart" data-page="1" class="blog-comment-nav-item">Начало</a> |
							<a href="<?= $arResult["NEW_PAGES"][$arResult['PAGE'][1]] ?>" id="blog-comment-nav-bprev">Пред. </a> |
						</span>
			<?
			for ($i = 1; $i <= $arResult["PAGE_COUNT"]; $i++) {
				...
				?>
				<a class="<?= $style ?>" data-page="<?= $i ?>" href="<?= $arResult["NEW_PAGES"][$i] ?>"
				   onclick="return bcNav('<?= $i ?>', this)" id="blog-comment-nav-b<?= $i ?>"><?= $i ?></a>
				<?
			}
			?>
			<span>
							| <a href="<?= $arResult["NEW_PAGES"][$arResult['PAGE']['PAGE_COUNT']] ?>"
							     id="blog-comment-nav-bnext">След. </a>
							| <a href="<?= $arResult["NEW_PAGES"][$arResult['PAGE']['PAGE_COUNT']] ?>"
							     data-page="<?= $arResult['PAGE_COUNT'] ?>"
							     onclick="return bcNav(<?= $arResult['PAGE_COUNT'] ?>, this)" id="blog-comment-nav-bend"
							     class="blog-comment-nav-item">Конец</a>
						</span>
		</div>
	<?
	}
	...
	<?
	if (!$USER->IsAdmin()) {
		echo "<p class='need_moderate_message'>Оставьте Ваш отзыв, он будет опубликован после проверки администрацией!</p>";
	}
	?>
		<div id="form_comment_" style="display:block;">
			<div id="form_c_del" style="display:block;">
				<div class="blog-comment-form test">
					<div class="blog-comment-form-error"></div>
						...
						<div class="review_page_main_block">
							<div class="author_page_inputs">
								...
								<? if (empty($arResult["User"])) { ?>
									<input class="author_page_input_name" maxlength="255" size="30" tabindex="3"
									       type="text" name="user_name" id="user_name"
									       value="<?= htmlspecialcharsEx($_SESSION["blog_user_name"]) ?>"
									       placeholder="Имя:">
									<input class="author_page_input_name" maxlength="255" size="30" tabindex="4"
								...
								<? if ($arParams["NOT_USE_COMMENT_TITLE"] != "Y") { ?>
									<div class="blog-comment-field blog-comment-field-title">
										<div class="blog-comment-field">
											...
											<span>
										<input size="70" type="text" name="subject" id="subject" value=""
										       placeholder="Имя:">
									</span>
											<div class="blog-clear-float"></div>
										</div>
										...
										<div class="blog-comment-field-captcha-image">
											<div id="div_captcha">
												<? if ($arResult["use_captcha"] === true) { ?>
													<div id="captcha_del">
														<img
															src="/bitrix/tools/captcha.php?captcha_code=<?= $arResult["CaptchaCode"] ?>"
															width="180" height="40" id="captcha" style="display:block;">
														<script>
								...
								<div class="author_page_input_capcha_send_block">
									<button class="author_page_button sprite_review_active"
									        onclick="submitComment()"></button>
								</div>
							</div>
						</div>
	...
	?>
		<div
			id="blog-comment-page-<?= $i ?>"<? if ($arResult["PAGE"] != $i) echo "style=\"display:none;\"" ?>><? RecursiveComments($tmp, $arResult["firstLevel"], 0, true, $arResult["canModerate"], $arResult["User"], $arResult["use_captcha"], $arResult["CanUserComment"], $arResult["COMMENT_ERROR"], $arResult["Comments"], $arParams); ?></div>
	<?
	}
	}
	else
	...
	?>
		<div class="blog-comment-nav">
			...
			<span>
							<a href="<?= $arResult["NEW_PAGES"][1] ?>" onclick="return bcNav('1', this)"
							   id="blog-comment-nav-tstart" data-page="1" class="blog-comment-nav-item">Начало</a> |
							<a href="<?= $arResult["NEW_PAGES"][$arResult['PAGE'][1]] ?>" id="blog-comment-nav-tprev">Пред. </a> |
						</span>
			<?
			for ($i = 1; $i <= $arResult["PAGE_COUNT"]; $i++) {
				$style = "blog-comment-nav-item";
				...
				?>
				<a class="<?= $style ?>" data-page="<?= $i ?>" href="<?= $arResult["NEW_PAGES"][$i] ?>"
				   onclick="return bcNav('<?= $i ?>', this)" id="blog-comment-nav-t<?= $i ?>"><?= $i ?></a>
				<?
			}
			?>
			<span>
							| <a href="<?= $arResult["NEW_PAGES"][$arResult['PAGE']['PAGE_COUNT']] ?>"
							     id="blog-comment-nav-tnext">След. </a>
							| <a href="<?= $arResult["NEW_PAGES"][$arResult['PAGE']['PAGE_COUNT']] ?>"
							     data-page="<?= $arResult['PAGE_COUNT'] ?>"
							     onclick="return bcNav(<?= $arResult['PAGE_COUNT'] ?>, this)" id="blog-comment-nav-tend"
							     class="blog-comment-nav-item">Конец</a>
						</span>
		</div>
	<?
...
<script>
	function showDots(page) {
		var max = <?=$arResult['PAGE_COUNT']?>;
		$('.blog-comment-nav-item').hide();
		$('.blog_comment_dots').remove();
		$('#blog-comment-nav-bprev').show();
		$('#blog-comment-nav-bnext').show();
		$('#blog-comment-nav-bstart').show();
		$('#blog-comment-nav-bend').show();
		$('.blog-comment-nav-item[data-page="1"]').show();
		$('.blog-comment-nav-item[data-page="2"]').show();
		$('.blog-comment-nav-item[data-page="3"]').show();
		$('.blog-comment-nav-item[data-page="<? echo $arResult['PAGE_COUNT'] - 2; ?>"]').show();
		$('.blog-comment-nav-item[data-page="<? echo $arResult['PAGE_COUNT'] - 1; ?>"]').show();
		$('.blog-comment-nav-item[data-page="<? echo $arResult['PAGE_COUNT']; ?>"]').show();
		if (page == max) {
			$('<span class="blog_comment_dots blog_comment_dots_before">...</span>').insertBefore('.blog-comment-nav-item[data-page="' + (max - 2) + '"]');
		} else if (page == 1) {
			$('<span class="blog_comment_dots blog_comment_dots_after">...</span>').insertAfter('.blog-comment-nav-item[data-page="' + 3 + '"]');
		} else {
			var i;
			for (i = page - 2; i <= page + 2; i++) {
				$('.blog-comment-nav-item[data-page="' + i + '"').show();
			}
			if (page > 6) {
				$('<span class="blog_comment_dots blog_comment_dots_before">...</span>').insertBefore('.blog-comment-nav-item[data-page="' + (page - 2) + '"]');
			}
			if (page < max - 6) {
				$('<span class="blog_comment_dots blog_comment_dots_after">...</span>').insertAfter('.blog-comment-nav-item[data-page="' + (page + 2) + '"]');
			}
		}
	}
	$(function () {
		$(document).on('click', '#blog-comment-nav-bprev, #blog-comment-nav-tprev', function () {
			var page = $('.blog-comment-nav-item-sel').first().data('page');
			showDots(page - 1);
			if (page > 1) {
				return bcNav(page - 1, this);
			} else {
				return bcNav(1, this);
			}
		});
		$(document).on('click', '#blog-comment-nav-bnext, #blog-comment-nav-tnext', function () {
			var page = $('.blog-comment-nav-item-sel').first().data('page');
			showDots(page + 1);
			if (page < <?=$arResult['PAGE_COUNT']?>) {
				return bcNav(page + 1, this);
			} else {
				return bcNav(<?=$arResult['PAGE_COUNT']?>, this);
			}
			return false;
		});
		$(document).on('click', '.blog-comment-nav-item', function () {
			showDots($(this).data('page'));
		});
		showDots(1);
		$(document).on('submit', '#form_comment', function () {
			BX.showWait('form_comment', 'Подождите');
			$.post($(this).attr('action'), $(this).serialize(), function (data) {
				BX.closeWait();
				if ($(data).find('.blog-note-error').html() != undefined) {
					$('.blog-comment-form-error').html($(data).find('.blog-note-error').html());
				} else {
					location.reload();
				}
			});
...
<span class="blog_comment_dots blog_comment_dots_before">...</span>