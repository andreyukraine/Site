...
<div class="bx_item_slider" id="<? echo $arItemIDs['BIG_SLIDER_ID']; ?>">
	...
	<span class="bx_bigimages_aligner">
		<? /*
			...
			$section = $section[0];
			$arSections = CIBlockSection::GetTreeList($section)->Fetch();
...
<?if(!empty($arResult['PROPERTIES']['PRICE_OLD']['VALUE'])) { ?> <s class="price-old"><?php echo site::ConvertPrice($arResult['PROPERTIES']['PRICE_OLD']['VALUE']), ' ', site::getCurrency(); ?></s> <?}?>

<?
$boolDiscountShow = (0 < $arResult['MIN_PRICE']['DISCOUNT_DIFF']);
?>
	<div class="item_old_price" id="<? echo $arItemIDs['OLD_PRICE']; ?>" style="display: <? echo ($boolDiscountShow ? '' : 'none'); ?>"><? echo ($boolDiscountShow ? site::ConvertPrice($arResult['MIN_PRICE']['PRINT_VALUE']) : ''); echo ' '.site::GetCurrency(); ?></div>
	<div class="item_current_price" id="<? echo $arItemIDs['PRICE']; ?>"><? echo site::ConvertPrice($arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE']); echo ' '.site::GetCurrency(); ?></div>
	<div class="item_economy_price" id="<? echo $arItemIDs['DISCOUNT_PRICE']; ?>" style="display: <? echo ($boolDiscountShow ? '' : 'none'); ?>"><? echo ($boolDiscountShow ? GetMessage('CT_BCE_CATALOG_ECONOMY_INFO', array('#ECONOMY#' => site::ConvertPrice($arResult['MIN_PRICE']['PRINT_DISCOUNT_DIFF']))) : ''); echo ' '.site::GetCurrency(); ?></div>
</div>
<?
...
?>
		...
		<? 
			if($arOneProp['ID'] == 44) {
				...
				echo "<a href='/catalog/?q={$autor}&s=author'>";
		}
		?>
		<?
			echo (
		...
		<dd>
			<?php
				...
				while($res_arr = $res->GetNext()){
					echo ($k++?' | ':'');
					if(!empty($res_keys['ITEMS']) || site::ConvertPrice($arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE'])=='0.00') echo '<a href="/upload/'.$res_arr['SUBDIR'].'/'.$res_arr["FILE_NAME"].'">';
					echo '<b>', preg_replace('~^.*\.~', '', $res_arr["FILE_NAME"]), '</b>';
					if(!empty($res_keys['ITEMS']) || site::ConvertPrice($arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE'])=='0.00') echo '</a>'; 
				}
			?>
		</dd>
		<?php } ?>
...
if ($arParams['USE_PRODUCT_QUANTITY'] == 'Y')
{
	if ($arParams['SHOW_BASIS_PRICE'] == 'Y')
	{
		$basisPriceInfo = array(
			'#PRICE#' => site::ConvertPrice($arResult['MIN_BASIS_PRICE']['PRINT_DISCOUNT_VALUE']).' '.site::GetCurrency(),
