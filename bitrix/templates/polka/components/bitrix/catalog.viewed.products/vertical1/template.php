...
if (!empty($arResult['ITEMS']))
{
	...
	<div class="bx_catalog_item_container <? echo $showImgClass; ?>">
			<? 	
				if(in_array('101', $arSections)) {
					echo "<div class='ebook_hover viewed_hover'>eBook</div>";
				}
			?>
	<a id="<? echo $arItemIDs['PICT']; ?>"
		href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"
		class="bx_catalog_item_images <? echo (in_array('101', $arSections))?'ebookimg':''; ?>"
	...
	<div class="bx_catalog_item_price">
		<div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price"><?
			if(!empty($arItem['PROPERTIES']['PRICE_OLD']['VALUE'])) { ?> <s class="price-old"><?php echo site::ConvertPrice($arItem['PROPERTIES']['PRICE_OLD']['VALUE']), ' ', site::getCurrency(); ?></s> <?}
			if (!empty($arItem['MIN_PRICE']))
			{
				...
				if ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $arItem['MIN_PRICE']['VALUE'])
				{
					...
					<? echo site::ConvertPrice($arItem['MIN_PRICE']['PRINT_VALUE']).' '.site::GetCurrency(); ?></span><?
				}
			}
			?></div>
	</div><?
