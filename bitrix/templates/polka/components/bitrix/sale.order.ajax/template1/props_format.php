			...
			?>
				<div>
					...
					foreach ($arSource as $arProperties)
					{
						...
						$no_address_ebook = '';
						// foreach ($arResult['BASKET_ITEMS'] as &$i) 
							// if(preg_match('~/ebooks/~', $i['DETAIL_PAGE_URL']) && in_array((int)$arProperties["ID"], array(7, 8))) {
							// if(preg_match('~/ebooks/~', $i['DETAIL_PAGE_URL']) && in_array((int)$arProperties["ID"], array(7, 8))) {
								// $no_address_ebook = ' style = "display: none;"';
								// break;
							// } unset($i);

						?>
						<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>"<?php echo $no_address_ebook; ?>>
							...
							<div class="bx_block r3x1">
								...
								<div class="location-block-wrapper">
									<?
										...
										$APPLICATION->IncludeComponent("bitrix:sale.location.selector.steps", "template1", $parametersProxed, null);
									?>
								</div>
								<? /*
