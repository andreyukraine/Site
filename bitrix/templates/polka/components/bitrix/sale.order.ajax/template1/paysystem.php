	...
	<div class="bx_section">
		<div class="order_page_payment_choose">
			...
			uasort($arResult["PAY_SYSTEM"], "cmpBySort"); // resort arrays according to SORT value
						?>
						<? //if($USER->IsAdmin()) {echo '<pre>'; print_r($arResult["PAY_SYSTEM"]); echo '</pre>';} ?>
						<?
						
			//проверяем если валюта грн. или руб то не выводим оплату пайпел
			if(site::GetCurrency() == 'UAH' ){
			...
			foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
			{
				if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
				{
					...
					else // more than one
					{
						...
						//echo '<xmp>'; print_r($arPaySystem); echo '</xmp>';
						if($arPaySystem['ID']==1 && empty($arResult['DELIVERY_SYSTEM'])) continue;
						?>
						<div class="bx_block horizontal">
							<div class="bx_element">
								<label for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
