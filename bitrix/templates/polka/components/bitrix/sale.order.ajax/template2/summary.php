...
?>
<div class="bx_ordercart order_page_inputs">
			...
			<thead>
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn) {
						if ($arColumn["id"] == "PROPS") $bPropsColumn = true;
						if ($arColumn["id"] == "NOTES") $bPriceType = true;
						if ($arColumn["id"] == "PREVIEW_PICTURE") $bPreviewPicture = true;
						if ($arColumn["id"] == "DETAIL_PICTURE") $bDetailPicture = true;
					}
			...
			<tbody>
				...
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
						...
						$arItem = (isset($arData["columns"][$arColumn["id"]])) ? $arData["columns"] : $arData["data"];
						// echo '<xmp style="text-align: left;">'; print_r($arItem); echo '</xmp>';
						// echo '<xmp style="text-align: left;">'; print_r($arColumn["id"]); echo '</xmp>';
						if ($arColumn["id"] == "NAME"):
						...
						?>
							<td class="price right">
								<div class="current_price"><? echo site::ConvertPrice($arItem["PRICE_FORMATED"]), ' ', site::getCurrency(); ?></div>
								<div class="old_price right">
									<?
									if (doubleval($arItem["DISCOUNT_PRICE"]) > 0):
										echo site::ConvertPrice(SaleFormatCurrency($arItem["PRICE"] + $arItem["DISCOUNT_PRICE"], $arItem["CURRENCY"])), ' ', site::getCurrency();
										$bUseDiscount = true;
									endif;
									?>
								</div>
						...
						<?
						elseif (in_array($arColumn["id"], array("QUANTITY", "DISCOUNT_PRICE_PERCENT_FORMATED", "SUM"))):
						?>
							<td class="custom right">
								<span><?=$arColumn?>:</span>
								<? 
									if($arColumn["id"]!='SUM') echo $arItem[$arColumn["id"]];
									else echo site::ConvertPrice($arItem[$arColumn["id"]]), ' ', site::getCurrency();
								?>
							</td>
						<?
						elseif (in_array($arColumn["id"], array("WEIGHT_FORMATED"))):
						?>
							<td class="custom right">
								<span><?=$arColumn?>:</span>
								<? echo number_format($arItem['WEIGHT'] / 1000, 2, '.', ' '), ' кг'; ?>
							</td>
						<?
			...
			<table class="bx_ordercart_order_sum">
				<tbody>
					<tr>
						...
						<td class="custom_t2" class="price">
							<?php 
							// echo '<xmp style="text-align: left;>'; print_r($arResult); echo '</xmp>'; ?>
							<?=number_format($arResult['ORDER_WEIGHT'] / 1000, 2, '.', ' ')?> кг
						</td>
					</tr>
					<tr>
						<td class="custom_t1" colspan="<?=$colspan?>" class="itog"><?=GetMessage("SOA_TEMPL_SUM_SUMMARY")?></td>
						<td class="custom_t2" class="price"><? echo site::ConvertPrice($arResult["ORDER_PRICE_FORMATED"]), ' ', site::getCurrency(); ?></td>
					</tr>
					<?
					if (doubleval($arResult["DISCOUNT_PRICE"]) > 0)
					{
						...
						<tr>
							...
							<td class="custom_t2" class="price"><? echo site::ConvertPrice($arResult["DISCOUNT_PRICE_FORMATED"]), ' ', site::getCurrency() ?></td>
						</tr>
						<?
					}
					...
					if (doubleval($arResult["DELIVERY_PRICE"]) > 0)
					{
						...
						<tr>
							...
							<td class="custom_t2" class="price"><? echo site::ConvertPrice($arResult["DELIVERY_PRICE_FORMATED"]), ' ', site::getCurrency(); ?></td>
						</tr>
						<?
					}
					...
					?>
						<tr>
							...
							<td class="custom_t2 fwb" class="price"><? echo site::ConvertPrice($arResult["ORDER_TOTAL_PRICE_FORMATED"]), ' ', site::getCurrency(); ?></td>
						</tr>
						<tr>
							<td class="custom_t1" colspan="<?=$colspan?>"></td>
							<td class="custom_t2" style="text-decoration:line-through; color:#828282;"><? echo site::ConvertPrice($arResult["PRICE_WITHOUT_DISCOUNT"]), ' ', site::getCurrency(); ?></td>
						</tr>
					<?
					else:
					?>
...
<? if($USER->IsAdmin()) {echo '<pre>'; print_r($arResult); echo '</pre>';} ?>
		...
		<div style="clear:both;"></div>
		<div class="bx_section order_page_review">
