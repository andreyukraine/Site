...
<?$APPLICATION->SetTitle("Букинист"); ?>
<?php
$arrFilter = array(
    'PROPERTY_114_VALUE' => 'buk'
);
if(isset($_POST['show_empty']) && in_array($_POST['show_empty'], array('Y', 'N'))) {
    setcookie("SHOW_EMPTY", $_POST['show_empty'], time()+3600);
}
if(isset($_POST['show_empty']) && in_array($_POST['show_empty'], array('Y', 'N'))) {
    $show_empty = $_POST['show_empty'];
} else if(isset($_COOKIE['SHOW_EMPTY'])) {
...
switch($show_empty){
    case 'Y': break;
    case 'N': $arrFilter["!CATALOG_QUANTITY"] = 0; break;
}

?>
<form action="" method="POST" name="is_empty" id="empty_form" style="margin: 20px 0; float: right;">
    <input type="hidden" name="show_empty" value="N">
    <label>
        <input type="checkbox" name="show_empty"<?php echo ($show_empty == 'Y')?" checked":""; ?> value="Y">
...
<?$APPLICATION->IncludeComponent(
    ...
    "template2",
    array(
        "ACTION_VARIABLE" => "news",	// Название переменной, в которой передается действие
        ...
        "ELEMENT_SORT_FIELD" => "PROPERTY_ARTNUMBER",	// По какому полю сортируем элементы
        ...
        "OFFERS_SORT_FIELD" => "id",	// По какому полю сортируем предложения товара
