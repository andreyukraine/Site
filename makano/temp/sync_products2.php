<?php 
	...
	function smarket_get_products(){
		...
		$query1 = "SELECT 
			...
			WHERE 
				...
				AND a.updated >= '".date('Y-m-d', strtotime('-15 day'))."'
			GROUP BY 
				a.nnt,
				a.kol_tmp,
				a.plu_cod,
	...
	function smarket_get_skidki(){
		...
		$query1 = "SELECT a.cardcode as COUPON, a.dis_percent FROM clients a 
				WHERE a.updated >= '".date("Y-m-d", strtotime('-15 day'))."'
			ORDER BY a.updated desc
		";
		$a = model('db', $config_smarket)->sql($query1); $max = 0;
		foreach ($a->items as &$i) {
			switch($i['dis_percent']){
				case 5: $i['DISCOUNT_ID'] = 2; break;
				case 6: $i['DISCOUNT_ID'] = 3; break;
				case 7: $i['DISCOUNT_ID'] = 4; break;
				case 8: $i['DISCOUNT_ID'] = 5; break;
				case 9: $i['DISCOUNT_ID'] = 6; break;
				case 10: $i['DISCOUNT_ID'] = 7; break;
				case 12: $i['DISCOUNT_ID'] = 8; break;
				case 14: $i['DISCOUNT_ID'] = 9; break;
				case 16: $i['DISCOUNT_ID'] = 10; break;
				case 18: $i['DISCOUNT_ID'] = 11; break;
				case 20: $i['DISCOUNT_ID'] = 12; break;
			}
			$max++;
		} unset($i);
		$a->max = $max;
	...
	function bitrix_get_orders(){
		...
		$query1 = "SELECT 
			...
			WHERE a.DATE_INSERT >= '".date("Y-m-d", strtotime('-15 day'))."'
	...
	<body class="container">
		<div class="content">
			...
			<ol>
				...
				<?php  ?>
				<li>Создание CSV ... <?php echo (create_csv($a, 'tovari')?'Ok':'Пусто'); ?></li>
				...
				<li>Экспорт купонов из S-market-a ... <?php $c = smarket_get_skidki(); echo ($c->max?'('.$c->max.') Ok':'Пусто'); ?></li>
				<li>Импорт купонов в Битрикс ... <?php echo (bitrix_import_skidki($c)?'Ok':'Пусто'); ?></li>
				<li>Экспорт клиентов из S-market-a ... <?php $d = smarket_get_clients(); echo ($c->max?'('.$c->max.') Ok':'Пусто'); ?></li>
